...
%	io:format("~p request for ~p with post: ~p, query: ~p~n", [Method, Splitted, Post, Query]),
	...
	try
		...
		{not_found, {_Object, _Id}} ->
			Req:respond(
				probix_error:response_error_as(json, 404, {Path, "no object with that id found"})
			);
		{bad_input, _Message} ->
			Req:respond(
				probix_error:response_error_as(json, 400, {Path, "bad input"})
			);
		_Exception ->
			Req:respond(
				probix_error:response_error_as(json, 500, {Path, "something bad happened"})
			)
	end.
...
handle('POST', [ "object", Id_string, "probes"], _, Post) ->
	...
	Json = probix_probe:create_from(json, Id, Post),
	{200, [{"Content-Type", "text/json"}], Json};
